...
admin-domain-register-success-local-no-amper = Domain <b>{$domain}</b> added for user (local only). To register via Amper set <code>AMPER_API_BASE_URL</code> and <code>AMPER_API_TOKEN</code> in .env on the server.
