...
service-dev-po = <b>💻 Development &amp; Infrastructure Solutions</b><br><br>Мы создаем и масштабируем цифровые продукты — от кода до трафика и инфраструктуры.<br><br><b>Что мы делаем:</b><br><br>🌐 <b>Веб-разработка</b><br>— Лендинги, высокопроизводительные сайты, SEO-ready структура<br>— Frontend: Next.js, React, Vue<br>— Чистая архитектура + быстрая загрузка + интерфейсы под конверсию<br><br>⚙️ <b>Backend &amp; Systems</b><br>— Node.js (NestJS / Express / Fastify), Python (FastAPI), PHP (Laravel)<br>— API-интеграции, аутентификация, биллинг, дашборды<br>— Масштабируемая backend-архитектура<br><br>🤖 <b>Автоматизация &amp; Боты</b><br>— Telegram / Discord боты<br>— Админ-панели, очереди, automation flow<br>— Кастомная backend-логика и интеграции<br><br>🧱 <b>Инфраструктура &amp; DevOps</b><br>— Настройка VPS / Dedicated (Nginx, Docker, CI/CD)<br>— Cloudflare, DNS routing, SSL, reverse proxy<br>— Безопасные деплои + скрытие origin<br><br>📊 <b>SEO &amp; Traffic-Ready Systems</b><br>— Техническое SEO (HTML-структура, schema, индексация)<br>— Аналитика (GA4, GTM, event tracking)<br>— Инфраструктура, подготовленная под привлечение трафика<br><br>🚀 <b>Запуск и масштабирование продукта</b><br>— От MVP до продакшена<br>— Архитектура, деплой, поддержка<br>— Системы, готовые к росту<br><br>Чтобы обсудить проект, нажмите кнопку ниже — откроется чат с поддержкой.<br>Отправьте подробный бриф: объем работ, функции, сроки и требования.<br>Мы изучим запрос и вернемся со структурированным предложением.
...
provisioning-user-ready-message = <strong>✅ Ваш выделенный сервер готов</strong>\n\n<strong>Тикет:</strong> <code>#{$ticketId}</code>\n\n<strong>Данные для доступа:</strong>\n{$message}\n\nЕсли потребуется помощь с настройкой — просто ответьте на это сообщение.
...
prime-subscription-body = Расширенные условия для активных клиентов и владельцев инфраструктуры.
 ...
 🏷 Скидка 10% на все доменные зоны
...
prime-subscription-benefit-discount = 🏷 Вам открывается дисконт в 10% на все доступные домена!
