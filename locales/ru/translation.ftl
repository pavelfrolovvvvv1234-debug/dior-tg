...
admin-domain-register-success-local-no-amper = Домен <b>{$domain}</b> добавлен пользователю (только в БД). Чтобы регистрировать через Amper, задайте <code>AMPER_API_BASE_URL</code> и <code>AMPER_API_TOKEN</code> в .env на сервере.
