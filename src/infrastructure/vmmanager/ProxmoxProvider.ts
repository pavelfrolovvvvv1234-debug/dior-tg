...
export class ProxmoxProvider implements VmProvider {
  ...
  async reconfigureVmResources(
    ...
    const current = await this.apiGet<{
      cores?: number;
      memory?: number;
      net0?: string;
      scsi0?: string;
    ...
    const targetCores = Math.max(1, Math.floor(Number(config.cpu) || 1));
    const targetMemoryMb = Math.max(512, Math.floor((Number(config.ramGb) || 1) * 1024));
    const targetDiskGb = Math.max(1, Math.floor(Number(config.diskGb) || 1));
    const targetNetworkMbit = Math.max(1, Math.floor(Number(config.networkMbit) || 0));

    const parseDiskSizeGb = (scsi0?: string): number | null => {
      const m = scsi0?.match(/size=([0-9.]+)([KMGTP])/i);
      if (!m) return null;
    ...
    const currentDiskGb = parseDiskSizeGb(current.scsi0);
    const net0 = current.net0;
    const net0WithRate =
      net0 && targetNetworkMbit > 0 && !/[, ]rate=\d+/i.test(net0)
        ? `${net0},rate=${targetNetworkMbit}`
        : net0;

    await this.apiPost(`/nodes/${this.node}/qemu/${id}/config`, {
      cores: targetCores,
      memory: targetMemoryMb,
      ...(net0WithRate ? { net0: net0WithRate } : {}),
    });

    if (currentDiskGb == null || targetDiskGb > currentDiskGb + 0.01) {
      await this.apiPost(`/nodes/${this.node}/qemu/${id}/resize`, {
        disk: "scsi0",
        size: `${targetDiskGb}G`,
      });
