...
- **Check** — для всех проверенных доменов (`example.com`, `test.com`, `test.lat`, `diorhost-check.com`) API возвращает `400 VALIDATION_ERROR: Invalid domain format. Example: example.com`. То есть запрос доходит, но валидация на стороне Amper не проходит.
